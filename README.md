...
./scheduler --config=/path/to/scheduler.yaml
./sender    --config=/path/to/sender.yaml
```
...
### Этапы разработки
Разработка разделена на следующие этапы:
...
#### Результатом выполнения следующих этапов является сервис «Календарь»:
- [Этап 1 «Заготовка сервиса Календарь»](./docs/12_README.md)
- [Этап 2 «Внешние API от Календаря»](./docs/13_README.md)
- [Этап 3 «Кроликизация Календаря»](./docs/14_README.md)
- [Этап 4 «Докеризация и интеграционное тестирование Календаря»](./docs/15_README.md)

Todo list:
 - реализовать логику выхода пользователя;
 - возвращать 409 "Conflict", если пользователь уже существует;
 - возвращать 404 "Not Found", если такого пользователя нет.
 - реализовать логирование для grpc.
 - реализовать health, info для grpc.
 - добавлять в контекст dockerfile только необходимые файлы.
 - переделать notify_before в секунды.